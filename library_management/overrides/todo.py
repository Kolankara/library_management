...
class CustomToDo(Document):
    def before_save(self):
        """
        Custom logic to be executed before saving the document.
        If the 'allocated_to' field is not set, it defaults to 'aathsha'.
        """
        ...
        if self.is_new():
            if self.assigned_by == self.allocated_to:
                ...
                assignment_message = frappe._("{0} assigned {1}: {2}").format(
                    ...
                    get_fullname(self.allocated_to), self.description
                )
            ...
            if self.get_db_value("status") != self.status:
                if self.allocated_to == frappe.session.user:
                    removal_message = frappe._(
                        "{0} removed their assignment.").format(
                        get_fullname(frappe.session.user)
                    )
                else:
                    removal_message = frappe._(
                        "Assignment of {0} removed by {1}").format(
...
def get_permission_query_conditions(user):
    ...
    if any(check in todo_roles for check in frappe.get_roles(user)):
        ...
        return """(`tabToDo`.allocated_to = {user} or 
        `tabToDo`.assigned_by = {user})""".format(
